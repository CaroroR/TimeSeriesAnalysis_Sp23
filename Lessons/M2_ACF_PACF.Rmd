...
ggplot(inflow_data, aes(x=my_date, y=Inflow)) +
            ...
            geom_line(aes(y=HP2,col="HP3")) +
            xlab("Time") +
            labs(color="Reservoir") #you can also do labs(color="")
```
...
```{r}
ts_inflow_data <- ts(inflow_data[,2:(2+nhydro-1)]) #note that we are only transforming columns with inflow data, not the date columns #2+nhydro-1 is just a more general way of writing if there are more columns of hydro plants and we don't know how many there are #[,"HPS"] or $HP2 also specify columns
ts_inflow_data
