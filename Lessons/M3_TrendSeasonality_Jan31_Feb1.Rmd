...
# coefficient of B1 in lm is level of the series, not average
...
Now let's shift attention to the seasonal component.The coefficient of the seasonal dummy is the mean of that season. for example, the mean of spring inflow.

Use seasonaldumy function.
...
ADF test first. null hypothesis states that there is a unit root, means it has stochastic trend. otherwise the dataset is stationary. so if it tells you the dataset has unit root, there is no longer need to fit a linear trend. If it tells you it is stationary, well it has no stochastic trend, but there are other deterministic trend. run Mann Kendall test afterwards.
